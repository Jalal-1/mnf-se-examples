    ...
    try {
      switch (choice) {
        ...
        case '3': {
          ...
          const txData = await api.balanceOf(contract, selfAccount);
          message = { text: `Balance query submitted (tx: ${txData.txId.substring(0, 16)}...)`, type: 'info' };
          break;
        }
        case '4': {
          // View total supply
          console.log(`\n    ${c.dim}Querying total supply (generates ZK proof)...${c.reset}`);
          const txData = await api.totalSupply(contract);
          message = { text: `Total supply query submitted (tx: ${txData.txId.substring(0, 16)}...)`, type: 'info' };
          break;
        }
        case '5':
